...
public class HistoryController {
	...
	public String listHistory(Model model,@PathVariable("id") int id,HttpSession session) {
		...
		List<HistoryDTO> list = historyService.getAllByProperty("maintenance.id", id);
		...
		model.addAttribute("submitForm", new HistoryDTO());
		return "history-list";
	}

	
	@PostMapping(value = {"/save/{id}"})
	public String saveHistory(Model model,@ModelAttribute("submitForm") HistoryDTO historyDTO ,@PathVariable("id") int id, HttpSession session ) {
		try {
